/*
  # Fix transaction details view running balance calculation

  1. Changes
    - Fix running balance calculation to only include tenant-related transactions
    - Explicitly define tenant transaction types:
      - Rent Charge/Payment
    ...
    - Remove lease fees and other owner transactions from balance calculation

  2. Technical Details
    - Use window functions for running balance calculation
    - Filter transaction types in CTE
    - Properly handle charges and payments
*/
...
running_balances AS (
  SELECT 
    t.*,
    CASE 
      WHEN t.is_tenant_transaction THEN
        SUM(CASE 
          WHEN t.is_tenant_transaction THEN t.amount 
          ELSE 0 
        END) OVER (
