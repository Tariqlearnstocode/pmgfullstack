-- Supabase test transactions SQL script
-- Run this in the Supabase dashboard SQL editor to add test transaction data

-- First, let's create a temporary starting point by adding a comment
SELECT 'Adding test transactions for reports testing' AS info;

-- Insert Rent Charges and Payments for John Doe
INSERT INTO public.transactions (id, tenant_id, property_id, type_id, amount, date, description, created_at)
VALUES 
...
INSERT INTO public.transactions (id, property_id, type_id, amount, date, description, created_at)
VALUES
...
-- (use the following if you have a starting_balance column in your tenants table)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tenants' AND column_name = 'starting_balance') THEN
    UPDATE public.tenants SET starting_balance = 0 WHERE id = '6dd87357-f55a-4020-92d3-97af4baf55ac';
    UPDATE public.tenants SET starting_balance = 0 WHERE id = '02404903-db89-4a03-8b2d-f311a1b79764';
    UPDATE public.tenants SET starting_balance = 200 WHERE id = 'ffdeba4d-dd9b-42c6-9045-1f69e9451200';
  END IF;
END $$;

-- Return count of inserted records as confirmation
SELECT 'Successfully inserted test transaction records' AS result, 
       (SELECT COUNT(*) FROM public.transactions WHERE date >= '2024-12-01') AS recent_transaction_count;
