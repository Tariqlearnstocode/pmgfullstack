/*
  ...
  1. Transaction Types
    ...
    - Lease Fee: One-time lease fee at move-in

  2. Transaction Pattern
    - Beginning of month: Rent charge
    - Middle of month: Rent payment
    - End of month: Management fee and lease fee

  3. Details
    - One year of transactions (12 months)
    - Consistent payment pattern
    - Management fees calculated on collected rent
    - Initial lease fee based on first month's rent
*/
...
WITH tenant_info AS (
  SELECT 
    ...
    tt_lease.id as lease_fee_type_id
  FROM tenants t
  ...
  JOIN transaction_types tt_lease ON tt_lease.name = 'Lease Fee'
...
FROM (
  ...
  SELECT 
    ...
    (date_trunc('month', NOW()) - interval '1 year' + (n || ' months')::interval)::date as date,
  ...
  SELECT 
    ...
    -monthly_rent as amount,
    (date_trunc('month', NOW()) - interval '1 year' + (n || ' months')::interval + interval '14 days')::date as date,
  ...
  SELECT 
    ...
    (date_trunc('month', NOW()) - interval '1 year' + ((n + 1) || ' months')::interval - interval '1 day')::date as date,
  ...
  -- Generate initial lease fee (First month only)
  SELECT 
    tenant_id,
    property_id,
    lease_fee_type_id as type_id,
    -(monthly_rent * (lease_fee_percentage / 100)) as amount,
    (date_trunc('month', NOW()) - interval '1 year')::date as date,
    'Initial Lease Fee' as description
  FROM tenant_info
) transactions
ORDER BY date;